...
const userBelongsToCommunity: CustomValidator = async (community, { req }) => {
...
export const validatePostCreation = [
  ...
  body('community').isString().withMessage('Community must be a string').custom(userBelongsToCommunity),
  body('summary')
    .optional()
    .isString()
...
export const createPost = async (req: Request, res: Response) => {
  try {
    ...
    if (!errors.isEmpty()) {
        ...
        const isCommunityError = errors.array().some(error => error.msg === 'User does not belong to the community');
        if (isCommunityError) {
